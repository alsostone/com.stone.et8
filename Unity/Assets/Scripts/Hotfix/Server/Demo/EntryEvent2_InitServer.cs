        ...
        protected override async ETTask Run(Scene root, EntryEvent2 args)
        {
            switch (Options.Instance.AppType)
            {
                case AppType.Server:
                {
                    ...
                    TbStartProcessRow tbStartProcessRow = TbStartProcess.Instance.Get(process);
                    if (tbStartProcessRow.Port != 0)
                    {
                        await FiberManager.Instance.Create(SchedulerType.ThreadPool, ConstFiberId.NetInner, 0, SceneType.NetInner, "NetInner");
                    }

                    // 根据配置创建纤程
                    var processScenes = TbStartScene.Instance.GetByProcess(process);
                    foreach (TbStartSceneRow startConfig in processScenes)
                    {
