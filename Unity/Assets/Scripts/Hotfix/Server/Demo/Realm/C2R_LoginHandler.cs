...
namespace ET.Server
{
	...
	public class C2R_LoginHandler : MessageSessionHandler<C2R_Login, R2C_Login>
	{
		...
		{
			// 随机分配一个Gate
			TbStartSceneRow row = RealmGateAddressHelper.GetGate(session.Zone(), request.Account);
			Log.Debug($"gate address: {row}");
			...
			G2R_GetLoginKey g2RGetLoginKey = (G2R_GetLoginKey) await session.Fiber().Root.GetComponent<MessageSender>().Call(
				row.ActorId, r2GGetLoginKey);

			response.Address = row.InnerIPPort.ToString();
