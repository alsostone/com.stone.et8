...
namespace ET
{
    ...
    public static class LSCommand
    {
        ...
        public static LSCommandData GenCommandFloat2(byte seatIndex, OperateCommandType type, float param1 = 0, float param2 = 0)
        {
            ...
            command.Header |= seatIndex << 24; // 前8位存储座位索引 服务器会覆盖
            command.Header |= (int)type << 16; // 接下来的8位存储操作类型
            command.Header |= version++ & 0xFF; // 最后8位存储版本号
        ...
        {
            byte type = ParseCommandSubType(command);
            return ((MovementMode)type, new TSVector2(command.Param1, command.Param2) / 1000);
        }
        ...
        public static (CommandButtonType, long) ParseCommandButton(LSCommandData command)
        {
            ...
            long param = (long)command.Param1 << 32;
            param |= (uint)command.Param2;
            return ((CommandButtonType)type, param);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static byte ParseCommandSubType(LSCommandData command)
        {
            return (byte)((command.Header >> 8) & 0xFF);
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static byte ParseCommandVersion(LSCommandData command)
        {
            return (byte)(command.Header & 0xFF);
        }
        ...
        public static (CommandGMType, long) ParseCommandGm(LSCommandData command)
        {
            ...
            long param = (long)command.Param1 << 32;
            param |= (uint)command.Param2;
            return ((CommandGMType)type, param);
        }
#endif
